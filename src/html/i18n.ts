...
type Messages = {
  ...
  // Which rows the control writes, BY NAME. An earlier version said "one
  // control of the product's screen, spelled by the 3 rows below" — true of
  // every control on every sheet, so furniture after one reading. Naming the
  // keys says the one thing indentation cannot: WHICH rows, and that they are
  // the ones directly beneath.
  compositeWrites: (keys: string[]) => string;
...
const ja: Messages = {
  ...
  compositeWrites: (keys: string[]) => `${keys.join(" / ")} を設定`,
...
const en: Messages = {
  ...
  compositeWrites: (keys: string[]) => `writes ${keys.join(" / ")}`,
