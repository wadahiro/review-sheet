...
// the path it lands at, `/etc/systemd/journald.conf.d/sample-platform.conf`, is
    ...
    if (specs.length > 0) {
      const read = specs.map((spec) => {
        ...
        return {
          ...
          // answer: `journald-sample-platform.conf.j2` is a bare `.conf`, which is
          // far too common a suffix to claim, while the artifact it becomes —
          // `/etc/systemd/journald.conf.d/sample-platform.conf` — says exactly
