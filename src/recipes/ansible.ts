...
export const ansibleRecipe: SheetRecipe = {
  ...
  load(sheetSpec, io: RecipeIO): SheetInputs {
    ...
    // own entries (`kc_vault_secrets[key=internal-ldap-bind].secret_name`), so the
    ...
    const loopMembers = (list: string, instance: string | undefined, component?: string): LoopMember[] => {
      ...
      // one (`[key=internal-ldap-bind]`) and by index otherwise — so they are read
    ...
    // static_files' embedded entries. The SCOPED ones are already folded into
    // rows that carry a value per environment — `layeredRecipe.load` does it,
    // for both recipes, because `static_files` is its field and the folding
    // reads nothing this recipe knows.
    const embedded: EmbeddedEntry[] = [...core.embedded];
        ...
        for (const entry of entries) {
          ...
          members.forEach((m, i) => {
            ...
            const made: Entry = {
              ...
              // SOURCE (`secret_list[key=internal-ldap-bind].secret_name`), which
