...
```jsonc
{
  ...
  "additional_sources": [
    { "file": "config/keycloak/app.yml", "path": "ssoSessionIdleTimeout",
...
defaults:
  ...
  key:
    from: path
    steps:
      - pattern: '^oidc\[name=oidc-app\]\.(.+)$'
        replace: 'oidc.$1'
      - pattern: '^saml\[name=saml-app\]\.(.+)$'
        replace: 'saml.$1'
```
...
component:
  map:
    SSO_LDAP_INTERNAL_PASSWORD: internal-ldap
```
...
assignment, and writing an assignment as a regex (`^SSO_LDAP_INTERNAL_PASSWORD$` ->
`internal-ldap`) only hides what it is.
...
part above is given the small defaults belonging to what it deploys. Give one a
ROLE-WIDE defaults instead — the file every template of that role shares — and
every variable in it that this part's templates never reference becomes a row of
its own, under its own name. Under `rows: artifact` too: the mode decides what a
row IS for the lines it renders, and does not narrow what the part answers for.

That is the "never lose a row" rule, and it is not ceremony — it is how a
variable with no file to be a line of reaches a sheet at all (an SELinux state a
command applies, a firewall opening: real rows on a real sheet, arriving by
exactly this path). A variable nothing here mentions is one of two things the
tool cannot tell apart — it belongs to another sheet, or a template that uses it
...
```yaml
    - recipe: ansible
      rows: artifact
      templates:                      # fixed-value policies, no {{ … }} in them
        - path: ../roles/os/templates/logrotate-app.j2
          deployed_path: /etc/logrotate.d/app
          component: /etc/logrotate.d/app
```

When the part DOES need some of a shared file, narrow it — and the report goes
quiet, because a filter is the author answering the question it asks:

```yaml
      defaults: ../roles/big_role/defaults/main.yml   # shared with other sheets
      include: ["app_*"]      # what this part reviews; the rest belongs
                              # to the sheet that uses it
```

`exclude: ["**"]` says the same thing the other way round when nothing here is
reviewable. Prefer either to `include:` with a pattern chosen to match nothing:
a filter that selects nothing is itself reported ("pattern matched nothing"), so
...
- name: keycloak clients
  ...
  split:
    ...
    only: [oidc-app, oidc-app-batch]   # optional: the members THIS sheet reviews
```

That one declaration does four things, and doing them separately is how they
drift apart:

- **the component** is the identity (`oidc-app`)
...
```yaml
  split:
    ...
    by: clientId
    members:
      - { id: saml-reports, match: 'https://*/reporting/saml/metadata' }
      - { id: saml-sp,        match: 'https://*/saml/metadata' }
      - { id: oidc-app }        # no match: — the id, literally
```
...
| `prefix` | the front of every one of its keys (`oidc-app.publicClient`), and the component slot is left to the source's own `component:` | **the component axis is already spent** — comparing two releases of a product whose file holds several clients |
...
(`clients[0].enabled` → `clients[clientId=oidc-app].enabled`), so the project
...
| document | `numbering:` | chapter numbers (1.2.3) beside each entry of the tree, on by default. DERIVED from declaration order and used for display only — anchors and links keep using names, so inserting a chapter moves the numbers and breaks no link (which is also why `--format md` names its directories rather than numbering them) |
...
a structural identity predicate (`clients[clientId=oidc-app].publicClient`,
...
```
  clients[clientId=oidc-app].publicClient → "Client: oidc-app" に割り当てました

  同じパターンの未解決キーが 6 件あります:
    clients[clientId=oidc-app].*
      clients[clientId=oidc-app].standardFlowEnabled
      clients[clientId=oidc-app].directAccessGrantsEnabled
      clients[clientId=oidc-app].rootUrl
      … 他 3 件
  すべて "Client: oidc-app" にしますか？ [y/N/l=一覧を全部見る]
```
