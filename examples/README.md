...
| [`ansible-keycloak/`](./ansible-keycloak/) | `import --spec` (`ansible` recipe) | single Keycloak role, reviewed as the **product's config keys** (`db-url`…, source-mapped back to the `kc_*` Ansible vars via the template); dictionary **extracted from the product** (Java reflection on the official image via the `keycloak-defaults-extractor` skill); build-time vs runtime scope; per-parameter out_of_scope secret |
