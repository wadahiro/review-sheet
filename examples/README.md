...
| [`ansible-keycloak/`](./ansible-keycloak/) | `import --spec` (`ansible` recipe) | single Keycloak role, **three sheets**: server config (`db-url`…), the realm it imports (`accessTokenLifespan`, `smtpServer.host`) and that realm's OIDC + SAML clients, all reviewed as the **product's own keys** and source-mapped back to the `kc_*` Ansible vars via their templates. Both dictionaries **extracted from the product** — the `PropertyMappers` registry (257 config keys, cross-checked against `kc.sh --help-all`) and `RealmRepresentation` + the admin console's own message bundles, whose **Japanese is Keycloak's** (74 bilingual entries nobody here translated). A `coverage: full` dictionary that deliberately is NOT materialized, and why; `sheets:`-namespaced project metadata; structural `path` source maps (nested JSON) beside `line`+`anchor` ones; rows addressed by **identity** (`id_fields: [clientId]` → `clients[clientId=corp-web].redirectUris`) and 27 bindings resolved through the `leaf` tier with no `dict_key` declared |
