...
describe("deriveBulkPattern — pattern derivation", () => {
  it("derives a pattern from a structural identity predicate (the oidc-app case from the task)", () => {
    expect(deriveBulkPattern("clients[clientId=oidc-app].publicClient")).toBe("clients[clientId=oidc-app].*");
  });

  it("cuts at the FIRST identity predicate, grouping deeper nesting under the same entity", () => {
    expect(deriveBulkPattern("clients[clientId=oidc-app].protocolMappers[name=aud].config.claim")).toBe(
      "clients[clientId=oidc-app].*"
    );
  });

  it("derives from a key that IS the predicate itself (no trailing field)", () => {
    expect(deriveBulkPattern("clients[clientId=oidc-app]")).toBe("clients[clientId=oidc-app].*");
  });
...
describe("matchesBulkPattern — literal prefix matching, never a real wildcard", () => {
  it("matches keys sharing the derived prefix", () => {
    const pattern = deriveBulkPattern("clients[clientId=oidc-app].publicClient")!;
    expect(matchesBulkPattern("clients[clientId=oidc-app].standardFlowEnabled", pattern)).toBe(true);
    expect(matchesBulkPattern("clients[clientId=oidc-app].redirectUris[0]", pattern)).toBe(true);
  });

  it("does not match a DIFFERENT entity's keys, even with an identical field name", () => {
    const pattern = deriveBulkPattern("clients[clientId=oidc-app].publicClient")!;
    expect(matchesBulkPattern("clients[clientId=saml-app].publicClient", pattern)).toBe(false);
  });
});

describe("runInteractiveSession — bulk apply", () => {
  const clientEntries = (extra: Partial<ScaffoldEntry> = {}): ScaffoldEntry[] => [
    entry({ sheet: "realm", key: "clients[clientId=oidc-app].publicClient", needsCategory: true, ...extra }),
    entry({ sheet: "realm", key: "clients[clientId=oidc-app].standardFlowEnabled", needsCategory: true, ...extra }),
    entry({ sheet: "realm", key: "clients[clientId=oidc-app].directAccessGrantsEnabled", needsCategory: true, ...extra }),
    entry({ sheet: "realm", key: "clients[clientId=saml-app].publicClient", needsCategory: true, ...extra }),
  ];
  ...
  it("presents the pattern, count, and matching keys BEFORE asking whether to apply", async () => {
    ...
    await runInteractiveSession(entries, { realm: ["Client: oidc-app", "Client: saml-app"] }, ask);

    const bulkQ = asked.find((q) => q.kind === "bulkApply");
    expect(bulkQ).toMatchObject({
      kind: "bulkApply",
      key: "clients[clientId=oidc-app].publicClient",
      pattern: "clients[clientId=oidc-app].*",
      category: "Client: oidc-app",
      matches: [
        "clients[clientId=oidc-app].standardFlowEnabled",
        "clients[clientId=oidc-app].directAccessGrantsEnabled",
      ],
    });
    // The other client (saml-app) is never offered here — different entity.
    expect((bulkQ as { matches: string[] }).matches).not.toContain("clients[clientId=saml-app].publicClient");
  });

  it("accepting ('y') resolves every matched entry with the same category, asking no further questions for them", async () => {
    const entries = clientEntries();
    const { ask, asked } = cannedAsk([
      "1", // publicClient -> Client: oidc-app
      "y", // bulk-apply to the other two oidc-app entries
      "2", // saml-app's own, unrelated, category question
    ]);
    const outcome = await runInteractiveSession(entries, { realm: ["Client: oidc-app", "Client: saml-app"] }, ask);

    expect(outcome.resolved).toEqual([
      { sheet: "realm", key: "clients[clientId=oidc-app].publicClient", category: "Client: oidc-app" },
      { sheet: "realm", key: "clients[clientId=oidc-app].standardFlowEnabled", category: "Client: oidc-app" },
      { sheet: "realm", key: "clients[clientId=oidc-app].directAccessGrantsEnabled", category: "Client: oidc-app" },
      { sheet: "realm", key: "clients[clientId=saml-app].publicClient", category: "Client: saml-app" },
    ]);
    expect(outcome.bulkApplied).toBe(2);
    // Only two category questions were asked in total (the triggering entry
    // and the unrelated saml-app one) — the two bulk-applied entries never
  ...
  it("skipping the triggering entry ('s') never offers a bulk apply — there is no category to apply", async () => {
    ...
    const entries = [
      entry({ sheet: "realm", key: "clients[clientId=oidc-app].publicClient", needsCategory: true }),
      entry({ sheet: "realm", key: "clients[clientId=saml-app].publicClient", needsCategory: true }),
    ];
    const { ask, asked } = cannedAsk(["s", "1"]);
    const outcome = await runInteractiveSession(entries, { realm: ["Client: oidc-app", "Client: saml-app"] }, ask);

    // No bulk offer follows the skip, since no category was ever picked for
    // clients[clientId=oidc-app].publicClient to propose applying elsewhere.
    expect(asked.filter((q) => q.kind === "bulkApply")).toHaveLength(0);
    expect(outcome.skipped).toHaveLength(1);
    expect(outcome.resolved).toEqual([{ sheet: "realm", key: "clients[clientId=saml-app].publicClient", category: "Client: oidc-app" }]);
  });
  ...
  it("description is resolved per-entry even when its category was bulk-applied", async () => {
    ...
    const { ask } = cannedAsk([
      "1",
      "y", // bulk-apply category to the other two oidc-app entries
