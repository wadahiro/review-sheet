...
// findings, or one that quietly refuses them.
...
function caps(...args: string[]): { review: boolean; prompt: boolean; code: number | null; stderr: string } {
  ...
  if (proc.exitCode !== 0) return { review: false, prompt: false, code: proc.exitCode, stderr: proc.stderr.toString() };
  const html = readFileSync(out, "utf-8");
  const m = /"review":(true|false),"prompt":(true|false)/.exec(html);
  if (m === null) throw new Error("no capability config in the generated HTML");
  return { review: m[1] === "true", prompt: m[2] === "true", code: proc.exitCode, stderr: proc.stderr.toString() };
}

describe("generate capability flags", () => {
  it("delivers review on when nothing is said", () => {
    expect(caps()).toMatchObject({ review: true });
  });

  it("hands over a document that can only be read", () => {
    expect(caps("--readonly")).toMatchObject({ review: false, prompt: false });
  });

  // The flag named only the review UI, but by the time the prompt existed it
  // meant neither of them. Renamed, not removed.
  ...
  // --allow is authoritative: naming only `prompt`-less `review` leaves the
  // prompt OFF, even though it is on by default. Anything else would make the
  // flag a set of additions rather than the whole permission set.
  it("treats --allow as the whole set, not an addition", () => {
    expect(caps("--allow", "review")).toMatchObject({ review: true });
    expect(caps("--allow", "")).toMatchObject({ review: false });
  });

  // The prompt is a judgement about the AUDIENCE. In the usual flow the review
  // goes back to whoever built the sheet and `apply` produces the prompt there,
  // against the real files — so the handed-over copy often has no use for one,
  // and shipping an affordance nobody asked to include is the thing --allow
  // exists to prevent.
  ...
  // The prompt is built FROM findings. A read-only document produces none, so
  // claiming the capability would describe a button that cannot exist — and a
  // config nobody can trust is worse than one feature fewer.
  ...
  it("refuses an unknown capability instead of ignoring it", () => {
    const r = caps("--allow", "review,reviw");
    expect(r.code).not.toBe(0);
