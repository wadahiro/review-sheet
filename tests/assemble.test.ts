...
// normalization filter that matches nothing (the project's case: filtering on
...
`,
    ...
    // all (no `dictionaries:` declared) — the exact shape of the project's T5
    ...
    // same as the project anecdote's starting point.
...
describe("SheetInputs.referenceSites: attaching additional_sources from a substitution scan", () => {
  ...
  it("a Pattern A row (base-only key) carries the sites", () => {
    const sites = [refSite("app.yml", "$(env:DB_HOST)")];
    const inputs: SheetInputs[] = [
      {
        name: "app",
        instances: [],
  ...
  it("an overlay-only key (never in base) carries the sites too", () => {
    const sites = [refSite("app.yml", "$(env:DB_PORT)")];
    const inputs: SheetInputs[] = [
      {
        name: "app",
        instances: ["staging"],
    ...
    const inputs: SheetInputs[] = [
      {
        ...
        layers: [{ kind: "base", entries: map([["db_host", entry("localhost")]]) }],
        embedded: [],
        referenceSites: [{ variable: "no_such_variable", sites: [refSite("app.yml", "$(env:X)")] }],
      },
    ];
...
`,
    ...
    const sites = [refSite("app.yml", "$(env:KC_DB_URL)")];
    const inputs: SheetInputs[] = [
      {
        name: "app",
        instances: [],
...
`;
    const input = assembleSheets(inputs("app"), opts());
    expect(pathsOf(input, "accessTokenLifespan")).toEqual([["app", "Tokens", "Access tokens"]]);
  });
