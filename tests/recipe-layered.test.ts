...
describe("layered recipe: same-file key collision (P3 — a TOML [[array-of-tables]] with a shared leaf name)", () => {
  ...
  const toml = [
    ...
    "[[saml]]",
    'name = "saml-app"',
    'base_url = "https://sp-dev.example.com"',
    "",
  ].join("\n");
  ...
  it("a POSITIONAL index is kept in the suggested replacement — dropping it would rebuild the collision", () => {
    // `[name=oidc-app]` can be dropped because the segment name itself still
  ...
  it("the thrown error names the colliding key, both structural paths, and a paste-able key-transform fix", () => {
    try {
      ...
      expect(message).toContain("oidc[name=oidc-app].base_url");
      expect(message).toContain("saml[name=saml-app].base_url");
  ...
  it("a per-source key transform (from: path) resolves the collision, keeping both rows distinct", () => {
    const si = layeredRecipe().load(
      {
        ...
        defaults: {
          ...
          key: {
            from: "path",
            steps: [
              { pattern: "^oidc\\[name=oidc-app\\]\\.(.+)$", replace: "oidc.$1" },
              { pattern: "^saml\\[name=saml-app\\]\\.(.+)$", replace: "saml.$1" },
            ],
          },
        },
      },
    ...
    const base = baseOf(si);
    expect(base.get("oidc.base_url")?.value).toBe("https://app-dev.example.com");
    expect(base.get("saml.base_url")?.value).toBe("https://sp-dev.example.com");
  });
  ...
  it("does NOT collide when the same shape is read as a static_file (keyed by structural path already)", () => {
    ...
    expect(si.embedded.map((e) => e.key).sort()).toEqual(["oidc[name=oidc-app].base_url", "saml[name=saml-app].base_url"]);
  });
    ...
    return layeredRecipe().load(
      {
        ...
        static_files: [{ path: "app.yml", ...(withSubstitution ? { substitution: { pattern: PATTERN } } : {}) }],
      },
      io
    );
  }
...
`;
  const files: Record<string, string> = {
    ...
    "/r/app.yml": "sslRequired: external\n",
  };
    ...
    const si = layeredRecipe().load(
      {
        ...
        recipe: "layered",
        defaults: "defaults.yml",
        static_files: [{ path: "master.yml", component: "master" }, { path: "app.yml" }],
      },
      io
    );
...
describe("layered recipe: a merged row keeps the component of the file that referenced it", () => {
    ...
    layeredRecipe().load(
      {
        ...
        defaults: "default.env",
        static_files: [
          { path: "app.yml", format: "yaml", component: "app", substitution: { pattern: "\\$\\(env:([A-Za-z_][A-Za-z0-9_]*)\\)" } },
  ...
  it("files the renamed base row under the referencing file's component", () => {
    ...
    expect(si.componentOf?.get("IDLE")).toBe("app");
  ...
  it("warns when two components merge the same variable, instead of picking one", () => {
      ...
      layeredRecipe().load(
        {
          ...
          static_files: [
            { path: "app.yml", format: "yaml", component: "app", substitution: { pattern: "\\$\\(env:([A-Za-z_][A-Za-z0-9_]*)\\)" } },
            // Same variable as app.yml's, from a different component.
    ...
    expect(warnings.join("\n")).toContain('"IDLE" is merged by two components (app, other)');
  });
});
...
describe("layered recipe: component map", () => {
  const files: Record<string, string> = { "/r/env.properties": "SSO_LDAP_INTERNAL_PASSWORD=x\nOTHER=y\n" };
  const io: RecipeIO = {
    readFile: (p) => files[p] ?? null,
    specDir: "/r",
    resolve: () => "/r/env.properties",
    instances: [],
    component: { map: { SSO_LDAP_INTERNAL_PASSWORD: "internal-ldap" } },
  };
  ...
  it("files a named key under the component it is assigned to", () => {
    ...
    expect(si.componentOf?.get("SSO_LDAP_INTERNAL_PASSWORD")).toBe("internal-ldap");
  ...
  it("warns about an assignment no row produced", () => {
    ...
    try {
      layeredRecipe().load(
        ...
        { ...io, component: { map: { SSO_LDAP_INTERNAL_PASSWORD: "internal-ldap", GONE: "internal-ldap" } } }
      );
    } finally {
      console.warn = spy;
    }
