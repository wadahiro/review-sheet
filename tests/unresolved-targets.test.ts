...
describe("unresolved review targets", () => {
  ...
  // would validate against a "app/General" that means something else.
  it("resolves a nested category by its full path", () => {
    const nested = data();
    nested.sheets[0].categories = [{ name: "app", categories: [{ name: "General", params: [] }] }];
    const ok: ReviewItem = { id: "c1", status: "pending", target: { sheet: "web", category: "app/General" }, changes: [] };
