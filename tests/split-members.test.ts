...
describe("split members: one name for two spellings", () => {
  ...
  it("matches an id with no match: literally", () => {
    expect(apply(SAML, "clients[clientId=oidc-app].protocol")).toBe("oidc-app.protocol");
  });
  ...
  it("tries members in the order written", () => {
    ...
    expect(apply(SAML, 'clients[clientId=https://app.example.com/reporting/saml/metadata].protocol')).toBe(
      "saml-reports.protocol"
    );
  });

  it("reports a member that recognised nothing", () => {
    const t = makeKeyTransformer({ from: "path", steps: splitKeySteps(SAML) });
    t.apply("clients[clientId=oidc-app].protocol");
    // saml-sp and saml-reports were never seen — a selection reviewing less
    // than it claims.
  ...
  it("drops it instead when the source IS the list", () => {
    ...
    expect(apply(SAML, "clients[clientId=oidc-app].protocol", true)).toBe("oidc-app.protocol");
  });
});
...
describe("split as:", () => {
  ...
  it("none drops the member identity entirely", () => {
    const one: StructuralSplit = { at: "components", by: "name", as: "none", members: [{ id: "internal-ldap" }] };
    expect(apply(one, "components[name=internal-ldap].config.enabled[0]")).toBe("config.enabled[0]");
  ...
  it("component (the default) names each member as itself", () => {
    ...
    expect(apply(c, "clients[clientId=oidc-app].protocol")).toBe("protocol");
  });
});
