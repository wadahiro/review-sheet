...
describe("what a Dockerfile decides", () => {
  ...
  // A row's value is a SUBSTRING OF ITS OWN LINE — that is what verify checks
  // and what apply writes back — so the exec form is stored as the file wrote
  // it. Canonicalising it here made the sheet show a value the file does not
  // contain, and verify said so on the next build.
  it("stores an exec form exactly as the file wrote it", () => {
