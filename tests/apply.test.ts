...
describe("a finding survives its row moving to another category", () => {
        ...
        categories: [
          {
            ...
            categories: [
              {
                name: categoryName,
                params: [{ key: "accessTokenLifespan", value: "300", source: { file: "app.yml", line: 3, anchor: "300" } }],
              },
            ],
          },
        ],
  ...
  const files: Record<string, string> = { "app.yml": "realm: app\naccessTokenLifespan: 300\n" };
  ...
  it("applies the change after the dictionary moved the row", () => {
    ...
    const out = computeApply(sheetWith("Tokens/Access tokens"), [review("app/Sessions")], read);
    expect(out.applied).toBe(1);
  ...
  it("reports the move rather than following it silently", () => {
    ...
    expect(out.moved).toEqual([
      { target: { sheet: "keycloak realm", category: "app/Sessions", param: "accessTokenLifespan" }, from: "app/Sessions", to: "app/Tokens/Access tokens" },
    ]);
  });

  it("says nothing when nothing moved", () => {
    const out = computeApply(sheetWith("Sessions"), [review("app/Sessions")], read);
    expect(out.applied).toBe(1);
    expect(out.moved).toEqual([]);
  });
      ...
      sheets: [
        {
          ...
          categories: ["app", "master"].map((c) => ({
            name: c,
            categories: [{ name: "Tokens", params: [{ key: "accessTokenLifespan", value: "300", source: { file: "app.yml", line: 3, anchor: "300" } }] }],
          })),
        },
      ],
    };
      ...
      sheets: [
        {
          ...
          categories: ["app", "master"].map((c) => ({
            name: c,
            categories: [{ name: "Tokens/Access tokens", params: [{ key: "accessTokenLifespan", value: c === "app" ? "300" : "60", source: { file: "app.yml", line: 3, anchor: "300" } }] }],
          })),
        },
      ],
    };
    const out = computeApply(data, [review("app/Sessions")], read);
    expect(out.applied).toBe(1);
    expect(out.moved[0].to).toBe("app/Tokens/Access tokens");
  });
});
