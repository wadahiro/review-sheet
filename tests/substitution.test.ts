...
// keycloak-config-cli's own syntax — the project's real-world example driving
...
function entry(key: string, value: string, path?: string): EmbeddedEntry {
  return { key, value, source: { file: "app.yml", line: 1, path: path ?? key } };
}
...
describe("bindReferences", () => {
  ...
  it("the project-shaped mix (whole-value + composed + literal + dangling) classifies every entry, dropping none", () => {
    const compiled = compileSubstitution(PATTERN);
